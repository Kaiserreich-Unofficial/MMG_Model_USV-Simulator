...
def run_simulation(max_trials, state_weight):
    # 将列表转为字符串，作为 launch 参数
    weight_str = "[" + ",".join(str(v) for v in state_weight) + "]"
    param_dict = {"StateWeight": weight_str}
    ...
    trial_iter = tqdm(range(1, max_trials + 1),
                      desc=f"StateWeight{state_weight}",
                      unit="trial")
...
def main():
    ...
    print(f"[INFO] 总共要跑 {len(state_weight_list)} 组状态权重，每组 {max_trials} 次仿真")

    try:
        for state_weight in state_weight_list:
            run_simulation(max_trials, state_weight)
    except KeyboardInterrupt:
