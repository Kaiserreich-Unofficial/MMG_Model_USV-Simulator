...
namespace heron
{
    ...
    public:
        ...
        {
            m = Iz = Xu_dot = Yv_dot = Nr_dot = 0.0f; // 初始化
        ...
        // 直接在 state 上更新
        __host__ __device__ void update(const float *tau_eff, float dt, FxTDOState &state)
        {
            float nu[3] = {this->observed_state[3], this->observed_state[4], this->observed_state[5]};
...
#pragma unroll
        ...
        // 观测器增益参数，由nh.param直接赋值
        float L1, L2;
        float k1, k1p, k1pp;
        float k2, k2p, k2pp;
        float d_inf;
        float observed_state[6]; // 观测状态，由host端赋值
