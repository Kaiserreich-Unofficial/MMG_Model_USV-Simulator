...
void mpc_timer_cb(const ros::TimerEvent &event_)
{
    ...
    Eigen::Vector3f d_hat = dynamics.fxtdo_.getDisturbance();
    std_msgs::Float32MultiArray d_hat_msg;
    d_hat_msg.data.resize(3);
    for (int i = 0; i < 3; ++i)
        d_hat_msg.data[i] = d_hat(i);
...
int main(int argc, char **argv)
{
    ...
    if (dynamics.enable_fxtdo_)
    {
        std::vector<float> L1_vec(3, 1.0f), L2_vec(3, 1.0f);
        std::vector<float> k1_vec(3, 3.0f), k1p_vec(3, 1.0f), k1pp_vec(3, 0.5f);
        std::vector<float> k2_vec(3, 2.0f), k2p_vec(3, 1.0f), k2pp_vec(3, 0.2f);
        std::vector<float> d_inf_vec(3, 0.3f);

        nh.getParam("fxtdo/L1", L1_vec);
        nh.getParam("fxtdo/L2", L2_vec);
        nh.getParam("fxtdo/k1", k1_vec);
        nh.getParam("fxtdo/k1p", k1p_vec);
        nh.getParam("fxtdo/k1pp", k1pp_vec);
        nh.getParam("fxtdo/k2", k2_vec);
        nh.getParam("fxtdo/k2p", k2p_vec);
        nh.getParam("fxtdo/k2pp", k2pp_vec);
        nh.getParam("fxtdo/d_inf", d_inf_vec);

        // 为每个方向单独设置参数
        for (int i = 0; i < 3; ++i)
            dynamics.fxtdo_.setObserverGains(i, L1_vec[i], L2_vec[i],
                                             k1_vec[i], k1p_vec[i], k1pp_vec[i],
                                             k2_vec[i], k2p_vec[i], k2pp_vec[i],
                                             d_inf_vec[i]);
    }
